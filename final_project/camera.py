...
class Cam(object):
    ...
    def get_blobs_upper(self, angle = 0) -> tuple:
        """
        Capture an image and detect colour blobs based on predefined thresholds.
        Region of interest is set to the bottom 2/3 of the image.
        Args:
        ...
        # Set region of interest (roi) to x=0, y=0, width=full width, height=440 pixels.
        blobs = img.find_blobs(self.thresholds, pixels_threshold=300, area_threshold=300,
                               roi=(0, 0, sensor.width(), 440))
    ...
    def find_colour_biggest_blob(self, blobs, threshold_idx: int):
        """
        Finds the biggest blob in blobs that was detected using a specified threshold
        ...
        for found_idx, colour in enumerate(colours):
            if colour == pow(2, threshold_idx):
                detected_blobs.append(blobs[found_idx])

        if detected_blobs:
            return self.get_biggest_blob(detected_blobs)
...
if __name__ == "__main__":
    ...
    sensor.set_auto_gain(False, gain_db = 12)  # can change the gain depending to the brightnes;
    ...
    cam = Cam(thresholds, 12)
    green_target_index=None
    blobs, img = cam.get_blobs_upper()
    green_target_index = cam.find_blob(blobs,0)
    print(green_target_index)
    ...
    while True:
        ...
        for blob in img.find_blobs(thresholds, pixels_threshold=400, area_threshold=3000):
